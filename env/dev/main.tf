...
module "bastion" {
  ...
  instance_type         = lookup(local.config, "bastion_instance_type", "t3.small")
  key_name              = lookup(local.config, "bastion_key_name", "ec2-kp")
...
module "rds" {
  ...
  backup_window          = lookup(local.config, "rds_backup_window", "03:00-04:00")
  maintenance_window     = lookup(local.config, "rds_maintenance_window", "sun:05:00-sun:06:00")
  parameter_group_family = lookup(local.config, "rds_parameter_group_family", "mysql8.0")

  cloudwatch_logs_exports      = lookup(local.config, "rds_cloudwatch_logs_exports", ["error", "slowquery"])
  performance_insights_enabled = lookup(local.config, "rds_performance_insights_enabled", false)
...
module "ecs" {
  ...
  keep_last_n_images      = lookup(local.config, "ecs_keep_last_n_images", 10)
